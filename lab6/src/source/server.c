...
uint64_t presentable = 1; // change this for right task
...
uint64_t Factorial(const struct FactorialArgs *args) {
    ...
    uint64_t res = MultModulo( args->begin, args->end, args->mod, presentable );
    return res;
}
            ...
            for (uint32_t i = 0; i < tnum; i++) {
                ...
                for (uint32_t i = 0; i < tnum; i++) {
                    ...
                    if (presentable) {
                        ...
                        total = MultModulo(total, result, mod, presentable);
                    }
                    printf("total%d %llu, result: %llu\n", i, total, result);
                }
